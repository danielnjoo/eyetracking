function [result, ivx]=IVXCalibration3(expt)
...
try
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    [result, ivx]=iViewX('stoprecording', ivx); % ?? necessary
    data = IVXreceive(ivx)
    ...
    [result, ivx]=iViewX('sendcalibrationpoints', ivx);
    ...
    if result==1
        ...
        while ~isempty(data);
            ...
            elseif strcmp(cmd,'ET_RES')
                'resultfully read back information about eye gaze during calibration'
    ...
    end % checkign whether calibration was resultful
